...
bestbooks <- allbooks %>% select(c(Title,Author,Published,Started,Finished,Pages,FicNonfic,Genre,Series,Category,BestCat,LinkedTitle,Image,Description)) 
...
best_table <- function(best_cat) {
  best_data <- bestbooks %>%  filter(str_detect(Category,best_cat)) %>% arrange(Author,Published)
text_tbl <- data.frame(
  Cover ='',
  Book = best_data$LinkedTitle,
  Author = best_data$Author,
  Description = best_data$Description)
image_table(text_tbl,best_data)
}
...
# Longer Lists 
## Best Books (Unranked) {.tabset .tabset-pills}
### Top 100 Fiction
```{r}
best_table("Fiction")
```
