...
booklist<-select(filter(allbooks, !is.na(Started)),c(Title,Author,Published,Started,Finished,Pages,FicNonfic,Genre))
...
### Unread
```{r}
select(filter(allbooks,(is.na(Started) | Order == "U") & Owned == 1),c(Title,Author,Pages,FicNonfic)) %>% arrange(Title) %>% datatable()
```

### Read
```{r}
select(filter(allbooks,(!is.na(Finished) | Order == "R") & Owned == 1),c(Title,Author,Pages,FicNonfic)) %>% arrange(Title) %>% datatable()
```